...
def getZooplaFlats(flatType,
  ...
  url = ZOOPLA + query.format(flatType, urlDistrict, minBeds, maxPrice,
                              minPrice, district)
  # print(url)
  r = requests.get(url)
  ...
  for res in soup.find_all('div'):
    if res.get('data-testid',
               '').startswith('search-result_listing') or res.get(
                 'data-testid', '').startswith('regular-listings'):
